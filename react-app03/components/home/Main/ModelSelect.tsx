  ...
  return (
    <div className="flex bg-gray-100 dark:bg-gray-900 p-1 rounded-xl">
      {models.map((item) => {
        ...
        const basicClass =
          "group hover:text-gray-900 hover:dark:text-gray-100 flex justify-center items-center space-x-2 py-2.5 min-w-[148px] test-sm font-medium border rounded-lg";
        const selectedClass =
          "border-gray-200 bg-white text-gray-900 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100";
        const notselectedClass = "border-transparent text-gray-500";
        return (
          <button
            key={item.id}
            className={`${basicClass} ${
              selected ? selectedClass : notselectedClass
            }`}
            onClick={() => {
              dispatch({
